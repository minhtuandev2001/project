...
const App = () => {
    ...
    // animation
    const [scaleItemID, setScaleItemID] = useState(null);
    const scaleAnimate = useRef(new Animated.Value(1)).current;

    // // handler scaleItem

    const handlerScaleItemIn = async (id) => {
        await setScaleItemID(id);
        await Animated.timing(scaleAnimate,
            {
                toValue: 1.04,
                duration: 100,
    ...
    const handlerScaleItemOut = async () => {
        await setScaleItemID(null);
        await Animated.timing(scaleAnimate,
            {
                toValue: 1,
                duration: 500,
    ...
    const item = ({ item }) => {
        return (
            <TouchableOpacity activeOpacity={0.9}
                ...
                onLongPress={() => handlerScaleItemIn(item.id)}
                onPressOut={() => handlerScaleItemOut()}
